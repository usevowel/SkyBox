...
body: "", full_url: 'ws://localhost:8787/ws'
            ...
            // ── Test 8: vm_on_http_request returns HTML ──
            const httpRequestData = JSON.stringify({
                method: 'GET', path: '/', matched_route: null,
                route_params: {}, raw_query: null, query: {},
                cookies: {}, headers: { host: 'localhost:8787' },
                body: "", full_url: 'http://localhost:8787/'
            });
            const httpResult = vm_on_http_request(httpRequestData);
            const httpParsed = JSON.parse(httpResult);
            if (httpParsed.status !== 200) throw new Error('Expected status 200, got ' + httpParsed.status);
            if (httpParsed.body.indexOf('<!DOCTYPE html>') === -1) throw new Error('Expected HTML in response body');
            if (httpParsed.body.indexOf('SkyBox Chat') === -1) throw new Error('Expected chat UI title in HTML');

            // ── Test 9: vm_on_http_request returns 404 for unknown paths ──
            const http404Data = JSON.stringify({
                method: 'GET', path: '/unknown', matched_route: null,
                route_params: {}, raw_query: null, query: {},
                cookies: {}, headers: { host: 'localhost:8787' },
                body: "", full_url: 'http://localhost:8787/unknown'
            });
            const http404Result = vm_on_http_request(http404Data);
            const http404Parsed = JSON.parse(http404Result);
            if (http404Parsed.status !== 404) throw new Error('Expected status 404, got ' + http404Parsed.status);
