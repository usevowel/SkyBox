...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            const rd = JSON.stringify({method:'GET',path:'/ws',matched_route:null,route_params:{},raw_query:null,query:{},cookies:{},headers:{upgrade:'websocket'},body:[],full_url:'ws://localhost:8793/ws'});
            vm_on_connect('c1',rd);
            msgs.length = 0;
            vm_on_message('c1',0,new TextEncoder().encode('analyze Hello world. Test sentence.'));
            const r1 = JSON.parse(msgs[0].text);
            if (!r1.analysis || r1.analysis.wordCount !== 4) return new Response('FAIL analyze: '+JSON.stringify(r1),{status:500});
            msgs.length = 0;
            vm_on_message('c1',0,new TextEncoder().encode('words hello hello world'));
            const r2 = JSON.parse(msgs[0].text);
            if (!r2.frequencies || r2.frequencies[0].count !== 2) return new Response('FAIL words: '+JSON.stringify(r2),{status:500});
            return new Response('OK: words='+r1.analysis.wordCount+' freq='+r2.frequencies[0].word);
